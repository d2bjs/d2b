...
Constructs a new scatter generator with the default settings. This is not the same as [d3.scatter](https://github.com/d3/d3-shape#scatter). The purpose of this module is to provide a common graph interface used in other types of graphs (e.g. scatter, line, bar).
...
# {#scatter_scatter}
[#](scatter.md#scatter_scatter) scatter.**scatter**([*d3-scatter*])

If *d3-scatter* is specified, sets the *d3-scatter* generator to the specified [d3-scatter](https://github.com/d3/d3-shape#scatter) and returns the d2b scatter generator. If *d3-scatter* is not specified, returns the current *d3-scatter* generator, which defaults to [d3.scatter()](https://github.com/d3/d3-shape#scatter).

The *d3-scatter* can be configured at will, except for the [x](https://github.com/d3/d3-shape#scatter_x) and [y](https://github.com/d3/d3-shape#scatter_y) properties which will be set automatically by the d2b scatter generator.
...
When the d2b scatter generator is applied to a selection, the following properties will be invoked. The function will be passed the element's bound [datum](https://github.com/d3/d3-selection#selection_datum) `d`.
...
When the d2b scatter generator is applied to a selection, the following properties will be invoked for each graph in the [graphs](#scatter_graphs) array. The function will be passed the graph data `d`.
...
If *x* is specified, sets the *x* scale to the specified accessor function or [d3-scale](https://github.com/d3/d3-scale) and returns the scatter generator. If *x* is not specified, returns the current *x* scale accessor, which defaults to a [d3.scaleScatterar()](https://github.com/d3/d3-scale#scaleScatterar).

# {#scatter_y}
[#](scatter.md#scatter_y) scatter.**y**([*y*])

If *y* is specified, sets the *y* scale to the specified accessor function or [d3-scale](https://github.com/d3/d3-scale) and returns the scatter generator. If *y* is not specified, returns the current *y* scale accessor, which defaults to [d3.scaleScatterar()](https://github.com/d3/d3-scale#scaleScatterar).
...
When the d2b scatter generator is applied to a selection, the following properties will be invoked for each element in the [values](#scatter_values) array. The function will be passed the value data `d`.
...
# {#scatter_py}
[#](scatter.md#scatter_py) scatter.**px**([*py*])
