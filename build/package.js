...
export var scripts = {"build:dev":"npm run build:package && npm run test && rollup -c -g d3:d3","build:prod":"npm run build:package && NODE_ENV=production rollup -c -g d3:d3","build:server":"http-server -a localhost -p 8000","build:watcher":"npm-watch","build:package":"json2module package.json > build/package.js","build:watch":"npm-run-all --parallel build:watcher build:server","start":"npm run build:server","test":"tape 'test/**/*.test.js'","docs:prepare":"gitbook install","docs:watch":"npm run docs:prepare && gitbook serve","docs:build":"npm run docs:prepare && rm -rf _book && gitbook build","docs:publish":"npm run docs:build && cp docs/cname _book && cd _book && git init && git commit --allow-empty -m 'Update docs' && git checkout master && git add . && git commit -am 'Update docs' && git push git@github.com:d2bjs/d2bjs.github.io master --force"};
export var dependencies = {"d3":"^4.5.0"};
